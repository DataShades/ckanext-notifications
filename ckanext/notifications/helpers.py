...
def get_unread_count_for_current_user():
    ...
    """
    context: Context = {"model": model, "session": model.Session, "user": tk.g.user, "auth_user_obj": tk.g.userobj}

    if not tk.g.userobj:
        return 0

    try:
        count = tk.get_action("notification_unread_count")(context, {"user_id": tk.g.userobj.id})
        return count
