...
def _format_email_body_for_notification(body: str, body_html: str):
    ...
    for match in URL_PATTERN.finditer(text):
        ...
        if trimmed_url:
            ...
            chunks.append(f'<a href="{href}" target="_blank" rel="noopener noreferrer">{label}</a>')
...
def cleanup_notifications_for_user(user_id: str):
    ...
    if max_notifications > 0:
        excess_ids_query = (
            model.Session.query(Notification.id)
...
def patch_ckan_flash():
    ...
    cast(Any, patched_flash)._is_patched_by_ext = True
...
def patch_ckan_mailer():
    ...
    cast(Any, patched_mail_recipient)._is_patched_by_ext = True
