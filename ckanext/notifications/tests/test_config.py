...
class TestConfigGetters:
    ...
    @pytest.mark.ckan_config(
        "ckanext.notifications.notifications_per_page", 0
    )
    ...
    @pytest.mark.ckan_config(
        "ckanext.notifications.notifications_per_page", -10
    )
    ...
    @pytest.mark.ckan_config(
        "ckanext.notifications.notifications_per_page", "invalid"
    )
    ...
    @pytest.mark.ckan_config(
        "ckanext.notifications.cleanup_days", 0
    )
    ...
    @pytest.mark.ckan_config(
        "ckanext.notifications.cleanup_days", "invalid"
    )
...
class TestConfigConstants:
    ...
    def test_constants_are_strings(self):
        ...
        assert isinstance(
            config.NOTIFICATIONS_FLASH_INTERCEPTION, str
        )
        assert isinstance(
            config.NOTIFICATIONS_DATASET_KEYWORDS, str
        )
        assert isinstance(
            config.NOTIFICATIONS_ORGANIZATION_KEYWORDS, str
        )
        assert isinstance(config.NOTIFICATIONS_GROUP_KEYWORDS, str)
        assert isinstance(
            config.NOTIFICATIONS_DATASET_ENDPOINT_STARTSWITH, str
        )
        assert isinstance(
            config.NOTIFICATIONS_RESOURCE_ENDPOINT_STARTSWITH, str
        )
        assert isinstance(
            config.NOTIFICATIONS_ORGANIZATION_ENDPOINT_STARTSWITH, str
        )
        assert isinstance(
            config.NOTIFICATIONS_GROUP_ENDPOINT_STARTSWITH, str
        )
    ...
    def test_default_constants_have_expected_types(self):
        ...
        assert isinstance(
            config.NOTIFICATIONS_DEFAULT_EMAIL_INTERCEPTION, bool
        )
        assert isinstance(
            config.NOTIFICATIONS_DEFAULT_FLASH_INTERCEPTION, bool
        )
        assert isinstance(
            config.NOTIFICATIONS_DEFAULT_DATASET_KEYWORDS, str
        )
        assert isinstance(
            config.NOTIFICATIONS_DEFAULT_PER_PAGE, int
        )
        assert isinstance(
            config.NOTIFICATIONS_DEFAULT_MAX_PER_USER, int
        )
        assert isinstance(
            config.NOTIFICATIONS_DEFAULT_CLEANUP_DAYS, int
        )
...
class TestConfigHelpers:
    ...
    def test_get_config_list_with_string_input(self):
        ...
        result = config._get_config_list(
            "test.key", "word1 word2 word3"
        )
        assert isinstance(result, list)
        assert len(result) == 3
