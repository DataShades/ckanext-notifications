...
class NotificationTestHelper:
    ...
    def create_notifications(user_id, count=5, **kwargs):
        ...
        for i in range(count):
            ...
            notif_kwargs["subject"] = notif_kwargs.get(
                "subject", f"Notification {i}"
            )
            notif = NotificationTestHelper.create_notification(
                user_id, **notif_kwargs
            )
    ...
    def get_notification_count(user_id, is_read=None):
        ...
        return len(
            NotificationTestHelper.get_user_notifications(user_id, is_read)
        )
